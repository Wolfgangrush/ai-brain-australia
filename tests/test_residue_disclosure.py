...
def test_write_audit_is_pii_free(tmp_path):
    from ailawfirm_australia.pseudonymisation_audit import write_audit
...
def test_au_tfn_is_masked():
    """AU: TFN must be masked."""
    gw = PseudonymisationGateway()
    clean, _ = gw.sanitize("The client's TFN 123 456 782 is on file.")
    assert "123 456 782" not in clean
