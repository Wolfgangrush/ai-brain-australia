...
Usage:
    from ailawfirm_australia.pseudonymisation import PseudonymisationGateway

    gw = PseudonymisationGateway()
    sanitized, token_map = gw.sanitize("Mr. John Smith filed a Federal Court writ...")
...
# ─────────────────────────────────────────────────────────────────────────
# Regex patterns — Australian TFN · ABN · Medicare · case numbers
# ─────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────
# Australia-native PII patterns (added v0.1.1 for jurisdictional coverage)
# ─────────────────────────────────────────────────────────────────────────

# TFN (Tax File Number): 9 digits, often space-grouped XXX XXX XXX
TFN_RE = re.compile(r"\bTFN[\s:]+\d{3}\s?\d{3}\s?\d{3}\b", re.IGNORECASE)

# Medicare: 10 digits, structured XXXX XXXXX X
MEDICARE_RE = re.compile(r"\bMedicare[\s:#]*\d{4}\s?\d{5}\s?\d\b", re.IGNORECASE)

# ABN (Australian Business Number): 11 digits, often grouped XX XXX XXX XXX
ABN_RE = re.compile(r"\bABN[\s:]+\d{2}\s?\d{3}\s?\d{3}\s?\d{3}\b", re.IGNORECASE)

# ACN (Australian Company Number): 9 digits, often grouped XXX XXX XXX
ACN_RE = re.compile(r"\bACN[\s:]+\d{3}\s?\d{3}\s?\d{3}\b", re.IGNORECASE)

# Australian phone: +61 prefix REQUIRED or 0-prefix mobile/landline format
# (no bare-digit match — avoids clobbering AADHAAR/general number sequences)
AU_PHONE_RE = re.compile(r"(?:\+61[\s-]?|0)(?:4\d{2}|[23478])[\s-]?\d{3,4}[\s-]?\d{3,4}\b")
...
# Australian case numbers: [YYYY] HCA/FCA/FCAFC/VSC/NSWSC/etc NNN
AU_CASE_RE = re.compile(
    r"\[?\d{4}\]?\s+(?:HCA|FCA|FCAFC|FCFCOA|VSC|NSWSC|QSC|WASC|SASC|TASSC|ACTSC|NTSC)\s+\d{1,4}",
    re.IGNORECASE,
)

# Australian BSB + account: BSB XXX-XXX, then account 9-10 digits
AU_BSB_RE = re.compile(r"\bBSB[\s:]*\d{3}[-\s]?\d{3}\b", re.IGNORECASE)


# (Note: Australian-specific patterns now consolidated in the v0.1.1 block above —
# TFN_RE / MEDICARE_RE / ABN_RE / ACN_RE / AU_PHONE_RE / AUD_AMOUNT_RE /
# AU_CASE_RE / AU_BSB_RE. Older greedy versions (post-anchor 4-digit postcode etc.)
# removed to avoid clobbering more-specific patterns like AADHAAR_RE.)
...
class PseudonymisationGateway:
    ...
    def __init__(self) -> None:
        ...
        self.patterns: list[tuple[re.Pattern, str]] = [
            # Australia-native (priority for Australia jurisdiction)
            (TFN_RE, "TFN"),
            ...
            (DATE_RE, "DATE"),
            (NAME_RE, "PERSON"),
            # (Note: Australian-specific patterns consolidated in the v0.1.1
            # priority block at the top of self.patterns — TFN/MEDICARE/ABN/
            # ACN/AU_BSB/AU_CASE/AU_PHONE/AUD_AMOUNT. Older greedy variants
            # removed to avoid clobbering Indian-diaspora AADHAAR detection.)
        ]
    ...
    def is_safe_for_cloud(text: str) -> tuple[bool, list[str]]:
        ...
        # Australian-specific checks (v0.1.1 — patterns require keyword context to
        # avoid clobbering AADHAAR detection in Indian-diaspora client matters)
        if TFN_RE.search(text):
            detected.append("TFN")
        if MEDICARE_RE.search(text):
            detected.append("MEDICARE")
        if ABN_RE.search(text):
            detected.append("ABN")
        if ACN_RE.search(text):
            detected.append("ACN")
        if AU_CASE_RE.search(text):
            detected.append("AU_CASE")
