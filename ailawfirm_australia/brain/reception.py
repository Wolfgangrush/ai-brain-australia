...
Prints a greeting banner, runs a systems check across the seven specialists
...
def run_reception() -> int:
    ...
    print("  G'day, Solicitor. Booting your brain...")
    print("======================================================")
    print()
    print("  SYSTEMS CHECK")
    print("  -------------")
    ...
    for name in AGENTS:
        try:
            mod = importlib.import_module(f"ailawfirm_australia.agents.{name}")
    ...
    router_ok = False
    try:
        router = importlib.import_module("ailawfirm_australia.brain.router")
