...
def handle(payload: str) -> dict:
    """Court agent entry point — keyword dispatch to ``lookup_court``.

    Scans the payload for an Australian court code (e.g. ``FCA``, ``NSWSC``),
    a jurisdiction token (``NSW``, ``VIC``, ``QLD``, ``WA``, ``SA``, ``TAS``,
    ``ACT``, ``NT``, ``federal``), or a court name fragment. Routes to
    ``lookup_court`` with the best-available filter. Always returns a dict
    that includes ``"agent": "court_agent"``.
    """
    ...
    upper = text.upper()
    # Match any known court code in the payload.
    matched_code = None
    for code in COURT_DATABASE:
        if re.search(rf"\b{re.escape(code)}\b", upper):
            matched_code = code
            break
    if matched_code is None:
        # Fallback: prefix/match against token boundaries.
        for code in COURT_DATABASE:
    ...
    for token, juris in jurisdiction_map.items():
        if re.search(rf"\b{token}\b", upper):
            matched_jurisdiction = juris
            break

    art_keywords = ("art", "aat", "administrative review", "tribunal")
    if any(k in text.lower() for k in art_keywords) and matched_code is None:
        matched_code = "ART"

    lookup_result = lookup_court(
        court_name=text if matched_code is None else None,
        court_code=matched_code,
        jurisdiction=matched_jurisdiction,
    )
