...
def handle(payload: str) -> dict:
    """Deadline agent entry point — delegating wrapper around ``handle_deadline``.

    Adds the canonical ``"agent": "deadline_agent"`` key (idempotently) and
    surfaces the v0.1 stub message alongside the standard limitation note.
    """
    ...
    result = handle_deadline(text)
    if isinstance(result, dict) and "agent" not in result:
        result["agent"] = "deadline_agent"
    return result
