...
def handle(payload: str) -> dict:
    """Compliance agent entry point — scan payload for LPUL/APP/Section 77/AML/Dayal risks.

    Delegates to ``check_compliance`` on the supplied payload and serialises
    the resulting ``ComplianceResult`` into a plain JSON-safe dict. Always
    returns a dict that includes ``"agent": "compliance_agent"``.
    """
